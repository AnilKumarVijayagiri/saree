...
export default function KurtiSets() {
  const [list, setList] = useState([]);
  useEffect(() => {
    (async () => {
      const { data } = await api.get('/api/products?category=Kurti Sets');
      setList(data);
    })();
  }, []);
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-center">Kurti Sets</h1>
