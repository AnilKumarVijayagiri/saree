...
export default function Gallery() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const uniqueCats = ["All", ...new Set(data.map((img) => img.category || "Uncategorized"))];
      setCategories(uniqueCats);
    })();
  }, []);

  const filteredImgs = imgs.filter((img) => {
    const inCategory = activeCategory === "All" || img.category === activeCategory;
    const inSearch =
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {filteredImgs.length > 0 ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full h-[400px] overflow-y-auto">
          {filteredImgs.map((img) => (
            <div
              key={img._id}
              className="group relative bg-white rounded-2xl shadow hover:shadow-lg overflow-hidden"
            >
              <img
                src={img.imageUrl}
                alt={img.title || ""}
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex flex-col justify-end p-4 transition">
                ...
                <p className="text-gray-200 text-sm">{img.category || "Uncategorized"}</p>
              </div>
            </div>
          ))}
        </div>
